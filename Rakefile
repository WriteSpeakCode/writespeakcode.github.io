...
# Copied from middleman-gh-pages gem until they allow to publish to master
...
PUB_BRANCH = 'master'
GH_PAGES_REF = File.join(BUILD_DIR, ".git/refs/remotes/#{remote_name}/#{PUB_BRANCH}")
...
file GH_PAGES_REF => BUILD_DIR do
  ...
  cd BUILD_DIR do
    ...
    if `git branch -r` =~ /PUB_BRANCH/
      sh "git checkout #{PUB_BRANCH}"
    else
      sh "git checkout --orphan #{PUB_BRANCH}"
      sh "touch index.html"
      sh "git add ."
      sh "git commit -m 'initial #{PUB_BRANCH} commit'"
      sh "git push #{remote_name} #{PUB_BRANCH}"
    end
  end
end
