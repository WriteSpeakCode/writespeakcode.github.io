		    ...
		    if (api.options.progress_bar && !vars.is_paused) $(vars.progress_bar).stop().css({left : -$(window).width()});
		  	...
		  	// Update slide caption
		   	if ($(vars.slide_caption).length){
		   		...
		   		(api.getField('pre')) ? $(vars.slide_pre).html(api.getField('pre')) : $(vars.slide_pre).html('');

		   		(api.getField('desc')) ? $(vars.slide_desc).html(api.getField('desc')) : $(vars.slide_desc).html('');

		   		(api.getField('link')) ? $(vars.slide_link).attr('href', api.getField('link')).css('display', 'inline-block') : $(vars.slide_link).hide();

		   		$('#slidenumber').html('Slide ' + (vars.current_slide + 1) + ' Of ' + api.options.slides.length);
		   	}
